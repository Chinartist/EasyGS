...
class GaussianModel():
    ...
    def create_from_pcd(self, xyz, rgb, scene_extent: float, add_skybox=False, skybox_points=100_000,
                        skybox_radius_scale=2.0):
        ...
        scales = torch.log(torch.clamp_max(torch.sqrt(dist2), self.scene_extent * self.percent_dense))[
            ..., None].repeat(1, 3)
        ...
        self._extra_attrs = nn.Parameter(
            torch.zeros((fused_point_cloud.shape[0], self._extra_attrs_dim), device="cuda").requires_grad_(True))
    ...
    def add_skybox(self, xyz, skybox_points=100_000, skybox_radius_scale=2.0):
        ...
        skyboxer = GaussianModel(init_sh_dgree=self.max_sh_degree, max_sh_degree=self.max_sh_degree,
                                 extra_attrs_dim=self._extra_attrs_dim, percent_dense=1e10)
        skyboxer.create_from_pcd(skybox_xyz.cpu().numpy(), fused_color.cpu().numpy(), scene_extent=self.scene_extent,
                                 add_skybox=False)
    ...
    def reset_opacity(self):
        opacities_new = self.opacity_inverse_activation(torch.min(self.opacity_activation(self._opacity),
                                                                  torch.ones_like(
                                                                      self.opacity_activation(self._opacity)) * 0.01))
    ...
    def load_ply(self, path):
        ...
        self._extra_attrs = nn.Parameter(
            torch.zeros((xyz.shape[0], self._extra_attrs_dim), device="cuda").requires_grad_(True))
    ...
    def densification_postfix(self, new_xyz, new_extra_attrs, new_features_dc, new_features_rest, new_opacities,
    ...
    def densify_and_split(self, grads, grad_threshold, N=4):
        ...
        selected_pts_mask = torch.logical_and(selected_pts_mask,
                                              torch.max(self.scaling_activation(self._scaling),
                                                        dim=1).values > self.percent_dense * self.scene_extent)
        ...
        self.densification_postfix(new_xyz, new_extra_attrs, new_features_dc, new_features_rest, new_opacity,
                                   new_scaling, new_rotation)
