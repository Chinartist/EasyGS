...
class Camera(nn.Module):
    def __init__(self, idx, R, T, FoVx, FoVy, focal_length_x, focal_length_y, height, width, image, depth, normal,
        ...
        if preload and isinstance(depth, str):
            ...
            self.depth_gt = \
            torch.nn.functional.interpolate(self.depth_gt[None, None], size=(height, width), mode="bilinear",
        ...
        if preload and isinstance(normal, str):
            ...
            self.normal_gt = \
            torch.nn.functional.interpolate(self.normal_gt[None], size=(height, width), mode="bilinear",
        ...
        if preload and isinstance(alpha, str):
            ...
            self.alpha_gt = \
            torch.nn.functional.interpolate(self.alpha_gt[None, None], size=(height, width), mode="nearest")[0, 0]
        else:
            self.alpha_gt = alpha
        ...
        if preload and isinstance(extra_attrs, str):
            ...
            self.extra_attrs_gt = \
            torch.nn.functional.interpolate(self.extra_attrs_gt[None, None].float(), size=(height, width),
        ...
        self.projection_matrix = getProjectionMatrix(znear=self.znear, zfar=self.zfar, fovX=self.FoVx,
                                                     fovY=self.FoVy).transpose(0, 1).cuda()  # c2image.T
        self.cuda()
    ...
    def get_depth_gt(self):
        if isinstance(self.depth_gt, str):
            return torch.nn.functional.interpolate(
                torch.tensor(np.array(np.load(self.depth_gt)), dtype=torch.float32)[None, None],
                size=(self.image_height, self.image_width), mode="bilinear", align_corners=False)[0, 0]
        else:
    ...
    def get_normal_gt(self):
        if isinstance(self.normal_gt, str):
            return \
            torch.nn.functional.interpolate(torch.tensor(np.array(np.load(self.normal_gt)), dtype=torch.float32)[None],
                                            size=(self.image_height, self.image_width), mode="bilinear",
                                            align_corners=False)[0]
        else:
    ...
    def get_alpha_gt(self):
        if isinstance(self.alpha_gt, str):
            alpha_gt = torch.nn.functional.interpolate(
                torch.tensor(np.array(Image.open(self.alpha_gt)), dtype=torch.float32)[None, None],
                size=(self.image_height, self.image_width), mode="nearest")[0, 0]
    ...
    def get_extra_attrs_gt(self):
        if isinstance(self.extra_attrs_gt, str):
            return torch.nn.functional.interpolate(
                torch.tensor(np.array(np.load(self.extra_attrs_gt)), dtype=torch.int16)[None, None].float(),
                size=(self.image_height, self.image_width), mode="nearest")[0, 0].long()
        else:
